  ...
  namespace :edi do
    ...
    task :send_ps, %i[org_code role] => [:load_app] do |_, args|
      ...
      id = if args.role.nil_or_empty? || args.role == 'marketer'
             repo.find_party_role_from_org_code(args.org_code, AppConst::ROLE_MARKETER)
           else
             repo.find_party_role_from_org_code(args.org_code, AppConst::ROLE_TARGET_CUSTOMER)
           end
    ...
    task :send_uistk, %i[org_code] => [:load_app] do |_, args|
      repo = MasterfilesApp::PartyRepo.new
      id = repo.find_party_role_from_org_code(args.org_code, AppConst::ROLE_MARKETER)
