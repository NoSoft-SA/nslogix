...
module MasterfilesApp
  class TestCommodityInteractor < MiniTestWithHooks
    ...
    def test_create_fail
      attrs = fake_commodity(commodity_code: nil).to_h.reject { |k, _| k == :id }
      res = interactor.create_commodity(attrs)
      refute res.success, 'should fail validation'
      assert_equal ['must be filled'], res.errors[:commodity_code]
    end

    def test_update
      id = create_commodity
      attrs = interactor.send(:repo).find_hash(:commodities, id).reject { |k, _| k == :id }
      value = attrs[:commodity_code]
      attrs[:commodity_code] = 'a_change'
      res = interactor.update_commodity(id, attrs)
      assert res.success, "#{res.message} : #{res.errors.inspect}"
      assert_instance_of(Commodity, res.instance)
      assert_equal 'a_change', res.instance.commodity_code
      refute_equal value, res.instance.commodity_code
    end

    def test_update_fail
      id = create_commodity
      attrs = interactor.send(:repo).find_hash(:commodities, id).reject { |k, _| %i[id commodity_code].include?(k) }
      value = attrs[:description]
      attrs[:description] = 'a_change'
      res = interactor.update_commodity(id, attrs)
      refute res.success, "#{res.message} : #{res.errors.inspect}"
      assert_equal ['is missing'], res.errors[:commodity_code]
